...
def f03(*words):
    truth = " ".replace(words)  # This is broken.
...
def f06(string1, string2):
    if len(string1) > len(string2):
        ...
        longer = string2
        shorter = string1
        print(float(len(shorter)))
        percentage = float(len(shorter)) / float(len(longer)) * 100
    print("{} is longer than {} by {} chars".format (longer, shorter, len(longer) - len(shorter)))
    print("{} has only {:2.2f}% the number of chars of {}".format(shorter, percentage , longer))
...
###############################################################################
def f09():
    summands = [n for n in range(1,500) if n % 3 == 0 or n % 5 == 0]
    return sum(summands)


###############################################################################
def f10(): # f10 should demonstrate recursion, returning the sum
    print("no idea")
...
def f12():
    ...
    lst = []
    try:
        if str(user_input) == "done" or str(user_input) == '"done"':
            f13()
            exit()
        else: 
    except:
        try:
            ...
            with open("log_file.txt", "w") as doc:
                doc.write(user_input + "\n")
